...
A paid live interoperability check passed through the production `evaluateJev` path with pinned model `jev-1.13.0`. A separate relevance benchmark used 12 synthetic intents against 95 cached tools and resources from six configured MCP servers. Semantic search achieved 90.9% Hit@1, 100% Hit@3, 95.5% mean reciprocal rank, and 100% no-match abstention, compared with 45.5% Hit@1/Hit@3/MRR for lexical search. The sole labeled top-rank miss returned the simulator-list resource ahead of the simulator-list tool, with the expected tool second. This is a small single-run benchmark over one local catalog, not a general quality guarantee. Live evaluation remains opt-in and is not part of normal installation or CI.
