...
- Added opt-in TypeSafe Jev semantic tool search and host-mediated `jev.evaluate` for `mcpScript`, with secure OS-keyring credentials, fixed-origin bounded requests, explicit data-egress policy, examples, and metadata-only traces. Live interoperability passed, and a 12-intent benchmark over 95 local MCP tools/resources reached 90.9% Hit@1 and 100% Hit@3 versus 45.5% for lexical search. Part of [#611](https://github.com/nicobailon/pi-mcp-adapter/issues/611). Thanks to [@nicobailon](https://github.com/nicobailon).
